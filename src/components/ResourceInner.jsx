...
import { faArrowLeft, faArrowUpRightFromSquare, faFilter, faStar } from "@fortawesome/free-solid-svg-icons";
...
function ResourceInner({h1, id, p, link, iconNameType, onBack, readTime, price, pageCount, iconName, summary, sneakTitle, sneakDescription, coreTopic, type}) {
          ...
          <div className="two-column-contain">
            <div className="left-resource-contain">
                ...
                <div className="sneak-contain">
                    <div className="row">
                        <h3 style={{color: 'var(--hoverBackgroundDark)'}}>Sneak peak</h3> 
                    {/* selects id based on resource then selects sneakitems and returns the length of that array */}
                    <div className='index-contain'>{resourcesData[id]?.sneakItems.length}</div>
                    </div>
                    
                    <div className="sneak-peak-wrapper">
                        {resourcesData[id]?.sneakItems.map((sneakItem, index) => {
                            const sneakInfo = SneakData.find((sneak)=> sneak.iconNameSneak === sneakItem.iconNameSneak)
                            return(
             ...
             <button className="main-button">
              ...
              <div className="right-resource-item">
                    ...
                    <p style={{color: 'var(--black)', fontWeight: 'bolder'}}><b>Other tags</b></p>
                ...
                <div className="right-resource-item">
                    ...
                    {resourcesData[id]?.pillarButtons.map((buttonInfo, index) => {
                       ...
                       // defined by each element (buttonInfo) defined under the pillarButtons array
                        const pillarInfo = PillarData.find((pillar) => pillar.iconName === buttonInfo.iconName);
                        return (
                            <PillarsButtons
                                key={index}
